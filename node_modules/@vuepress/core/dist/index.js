...
//#endregion
//#region src/app/prepare/prepareUserStyle.ts
/**
* Generate user style temp file
*
* @internal
*/
const prepareUserStyle = async (app, fileExists = null) => {
	let content = "";
	const { userStyle } = app.options;
	if (userStyle) {
		const userStylePath = app.dir.source(userStyle);
		if (fileExists ?? await fs.pathExists(userStylePath)) content += `import ${JSON.stringify(userStylePath)}\n`;
	}
...
const resolveAppMarkdown = async (app) => {
	...
	if (app.options.markdown.assets !== false) {
		...
		app.options.markdown.assets.absolutePathPrependBase ??= app.options.bundler.type === "webpack";
	}
...
* @internal
*/
const resolveAppOptions = ({ base = "/", lang = "en-US", title = "", description = "", head = [], locales = {}, source, dest = path.resolve(source, ".vuepress/dist"), temp = path.resolve(source, ".vuepress/.temp"), cache = path.resolve(source, ".vuepress/.cache"), public: publicDir = path.resolve(source, ".vuepress/public"), host = "0.0.0.0", port = 8080, open = false, templateDev = path.normalize(require$1.resolve("@vuepress/client/templates/dev.html")), shouldPreload = true, shouldPrefetch = "as-needed", templateBuild = path.normalize(require$1.resolve("@vuepress/client/templates/build.html")), templateBuildRenderer = templateRenderer, bundler, debug = false, markdown = {}, pagePatterns = ["**/*.md", "!.vuepress"], permalinkPattern = null, userStyle = null, plugins = [], theme }) => {
	locales["/"] ??= {
		lang,
		title,
		description
...
const setupAppThemeAndPlugins = (app, config) => {
	...
	app.options.userStyle = config.userStyle ?? themeInfo.userStyle ?? app.options.userStyle;
...
//#endregion
export { DEFAULT_IGNORE_PATTERNS, appInit, appPrepare, appUse, createAppDirFunction, createBaseApp, createBuildApp, createDevApp, createHookQueue, createPage, createPluginApi, createPluginApiHooks, createPluginApiRegisterHooks, inferPagePath, normalizeAliasDefineHook, normalizeClientConfigFileHook, parsePageContent, prepareClientConfigs, preparePageChunk, prepareRoutes, prepareSiteData, prepareUserStyle, renderPageToVue, resolveAppDir, resolveAppEnv, resolveAppOptions, resolveAppPages, resolveAppSiteData, resolveAppVersion, resolveAppWriteTemp, resolvePageChunkInfo, resolvePageContent, resolvePageDate, resolvePageFilePath, resolvePageHtmlInfo, resolvePageLang, resolvePagePath, resolvePagePermalink, resolvePageRedirects, resolvePageRouteMeta, resolvePageSlug, resolvePluginObject, resolveThemeInfo };
