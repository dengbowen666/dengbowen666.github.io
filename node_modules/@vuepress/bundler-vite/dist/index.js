...
import { build, createServer, mergeConfig, mergeConfig as mergeConfig$1 } from "vite";
import { createPageChunkFilesMap, createVueServerApp, getSsrTemplate, renderPageToString, resolveLinkedPageChunkFiles } from "@vuepress/bundlerutils";
...
const resolveViteConfig = ({ app, options, isBuild, isServer }) => {
	let viteConfig = mergeConfig$1({
		clearScreen: false,
		configFile: false,
	...
	const configureViteResult = options.configureVite?.(viteConfig, isServer, isBuild);
	if (configureViteResult) viteConfig = configureViteResult;
	return viteConfig;
};
//#endregion
...
*/
const renderPagePrefetchLinks = ({ app, outputEntryChunk, page, pageChunkFiles, pageChunkFilesMap }) => {
	...
	if (shouldPrefetch === "as-needed") {
		...
		candidateFiles = outputEntryChunk.dynamicImports.filter((item) => linkedPageChunkFiles.has(item) && !pageChunkFiles.some((file) => file === item));
	} else candidateFiles = outputEntryChunk.dynamicImports.filter((item) => !pageChunkFiles.some((file) => file === item));
	return candidateFiles.map((item) => {
		const type = item.endsWith(".js") ? "script" : item.endsWith(".css") ? "style" : "";
		if (shouldPrefetch !== true && shouldPrefetch !== "as-needed" && !shouldPrefetch(item, type)) return "";
...
const renderPage = async ({ app, page, vueApp, vueRouter, ssrTemplate, output, outputEntryChunk, outputCssAsset, pageChunkFilesMap }) => {
...
const build$1 = async (options, app) => {
	...
	await withSpinner(`Rendering ${app.pages.length} pages`)(async (spinner) => {
		...
		const pageChunkFilesMap = createPageChunkFilesMap({
			pages: app.pages,
			resolvePageChunkFiles: (page) => resolvePageChunkFiles({
				page,
...
//#endregion
export { src_default as default, viteBundler, mergeConfig as viteMergeConfig };
