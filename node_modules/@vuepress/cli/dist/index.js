...
import { DEFAULT_IGNORE_PATTERNS, createBuildApp, createDevApp, createPage, preparePageChunk, prepareRoutes, prepareUserStyle } from "@vuepress/core";
...
const watchPageFiles = (app) => {
	...
	const pagePromises = /* @__PURE__ */ new Map();
	...
	const processPageEvents = async (filePathRelative) => {
	...
	const pageEventHandler = (filePathRelative, eventType) => {
		let events = pendingEvents.get(filePathRelative);
		if (!events) pendingEvents.set(filePathRelative, events = []);
		events.push(eventType);
		const existingPromise = pagePromises.get(filePathRelative) ?? Promise.resolve();
		const newPromise = (async () => {
			await existingPromise;
			try {
				await processPageEvents(filePathRelative);
			} catch (error) {
				logger.error(`Error while processing page events for ${colors.magenta(filePathRelative)}:`, error);
			}
		})().finally(() => {
			if (pagePromises.get(filePathRelative) === newPromise) pagePromises.delete(filePathRelative);
		});
		pagePromises.set(filePathRelative, newPromise);
	};
	const depsListener = (dep) => {
...
//#endregion
//#region src/commands/dev/watchUserStyleFile.ts
const watchUserStyleFile = (app) => {
...
const dev = async ({ defaultAppConfig, sourceDir = ".", cliOptions = {} }) => {
	...
	if (app.options.userStyle) watchers.push(watchUserStyleFile(app));
...
//#endregion
export { build, cli, createPageDepsHelper, defineUserConfig, dev, handlePageAdd, handlePageChange, handlePageUnlink, info, loadUserConfig, mergeEvents, processPagePatterns, resolveAppConfig, resolveCliAppConfig, resolveConfig, resolveUserConfig, resolveUserConfigConventionalPath, resolveUserConfigPath, transformUserConfigToPlugin, watchPageFiles, watchUserConfigFile, watchUserStyleFile };
